    ...
    if (formData.images.length) {
      formData.images.forEach((image, i) => {
        form.append(
          "images[]",
          image,
          new Date().getTime() + i + "." + image.type.split("/")[1]
        );
      });
    }
    ...
    return (
        ...
        >
          ...
          {isAdmin === true ? (
            <div className="form-group">
              ...
              {submitted &&
                !customerId && (
                  <div className="help-block">Glöm inte välja rätt kund!</div>
                )}
            </div>
          ) : (
            <input
	  ...
	  <label htmlFor="description">Beskrivning av ärendet</label>
              <textarea
                ...
                name="description"
                placeholder="Beskriv gärna detaljerat vad som ska åtgärdas. Bifoga bilder vid behov"
                value={description}
                onChange={this.handleChange}
              />
            {submitted &&
